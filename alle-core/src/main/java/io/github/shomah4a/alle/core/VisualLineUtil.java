...
public final class VisualLineUtil {
     ...
     */
    public static ImmutableList<Integer> computeVisualLineBreaks(String lineText, int columns) {
     ...
     * @return 視覚行数（1以上）
     */
    public static int computeVisualLineCount(String lineText, int columns) {
        if (columns <= 0 || lineText.isEmpty()) {
            return 1;
        }
        return computeVisualLineBreaks(lineText, columns).size() + 1;
    }
     ...
     * @return 視覚行番号（0始まり）
     */
    public static int computeVisualLineForOffset(String lineText, int columns, int cpOffset) {
        if (columns <= 0 || lineText.isEmpty()) {
            return 0;
        }
        var breaks = computeVisualLineBreaks(lineText, columns);
        int visualLine = 0;
     ...
     */
    public static int visualLineStartOffset(String lineText, int columns, int visualLine) {
     ...
     */
    public static int visualLineEndOffset(String lineText, int columns, int visualLine) {
        if (columns <= 0 || lineText.isEmpty()) {
            return (int) lineText.codePoints().count();
        }
        var breaks = computeVisualLineBreaks(lineText, columns);
