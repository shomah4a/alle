...
ユーザーが `init.py` 等から設定値を切り替えられるよう、以下を本 ADR の対応範囲に含める。

- `SettingsRegistry` に文字列キーアクセス API
  （`setGlobalByKey` / `getEffectiveByKey` / `removeGlobalByKey`）を追加
- `EditorFacade` に `setSetting` / `getSetting` / `removeSetting` を追加
- Python 側に `alle.settings` モジュールを新設し、
  `set_global` / `get_effective` / `remove_global` を提供
- 型チェックは登録済み `Setting<T>` の `Class.cast` による実行時検証
