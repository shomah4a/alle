...
プロトコル行の受理範囲は ADR 0141 (Jackson 3 アップグレード) で厳格化された
(後続トークン付きの行は不正行として扱う)。
...
JSON Lines 形式のプロトコルを採用する。Jackson を用いてエンコード/デコードする。
拡張性があり、パスに特殊文字を含む場合も安全にエスケープされる。
...
- `alle` (引数なし or ファイルパス): 通常のエディタ起動。`M-x server-start` でサーバーを起動する
