...
public final class FaceName {
    ...
    public static final FaceName NUMBER =
        new FaceName("number", "数値リテラル");
    public static final FaceName ANNOTATION =
        new FaceName("annotation", "アノテーション・デコレータ");
...
- スクリプトから `new FaceName("custom-face", "カスタム用途")` のように拡張可能
...
- 既存のFace型を廃止し、StylingRule、スタイラー、FaceResolver、テキストプロパティfaceをFaceNameベースに移行済み
- FaceNameの追加は今後のモード拡張時に必要になる
