...
FilePathCompleter のコンストラクタに homeDirectory パラメータを追加し、
補完候補の ~ 表示を FilePathCompleter 自身が行う設計とした。
これにより TreeDiredCommand, SaveBufferCommand にも homeDirectory の伝播が必要となった。
...
- FindFileCommand: 初期値の ~ 表示、パス解決時の ~ 展開、homeDirectory パラメータ追加
- FilePathCompleter: ~ パスの補完候補生成、シャドウ対応、homeDirectory パラメータ追加
- SaveBufferCommand: ~ 展開、homeDirectory パラメータ追加
- TreeDiredCommand: 初期値の ~ 表示、~ 展開、homeDirectory パラメータ追加
- TreeDiredInitializer: homeDirectory パラメータ追加
- EditorCore: homeDirectory の受け渡し
- PathResolver: 新規ユーティリティクラス（collapseTilde, expandTilde, findShadowBoundary）
- FaceName: FILE_NAME_SHADOW の追加
- Completer: シャドウ境界判定のデフォルトメソッド追加
- MinibufferInputPrompter: シャドウ face の適用と確定時の除去、バックスペースコマンド追加
- DefaultFaceTheme: FILE_NAME_SHADOW face のスタイル定義（black_bright）
