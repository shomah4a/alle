...
- TreeDiredMode: 11コマンドをモードスコープに移行（tree-diredコマンド自体はグローバルに残す）
- PythonMode: 3コマンド（python-indent-line, python-dedent-line, python-newline-and-indent）をモードスコープに移行
- ElectricPairMode: 8コマンド（electric-open-paren等）をモードスコープに移行
- Python側のモード基底クラス（MajorModeBase/MinorModeBase）にcommands()メソッドを追加
- ブリッジ層（internal/mode.py）でcommands()からCommandRegistryへの変換を実装
