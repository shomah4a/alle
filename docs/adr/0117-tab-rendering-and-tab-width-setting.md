...
**タブストップの基準**: 各視覚行の先頭を 0 カラムとする「視覚行ローカル基準」とする。折り返しが発生した場合、折り返し後のタブは新しい視覚行の先頭から数えた `tabWidth` 境界に整列する。非折り返し行および水平スクロール時は `lineText` 先頭からの絶対カラムと一致する。

理由: 絶対カラム基準だと、折り返し後にタブが現れた際の表示幅が視覚行内の位置と無関係に決まり、視覚的に不自然になる。また `computeVisualLineBreaks` のループ内でカラムをリセットする既存構造と一致する。
